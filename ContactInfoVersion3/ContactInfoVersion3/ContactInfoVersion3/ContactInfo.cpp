	...
	if (OCRFile.is_open()) {
		...
		while (getline(OCRFile, mysteryContactInfo)) {
			...
			if (regex_match(mysteryContactInfo, regex("[A-Z]([a-z]|-[A-Z]|-[a-z]|')+ [A-Z]([a-z]|-[A-Z]|')+"))) {
				...
				//Version 3 is scanning a hypothetical database with lists of occupations
				ifstream Occupations;
				bool unique = true;
				string occupation;
				Occupations.open("Occupations.txt");
				while (getline(Occupations, occupation)) {
					if ( occupation == mysteryContactInfo) {
						unique = false;
						break;
					}
				}
